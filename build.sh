...
# Bazzite game setup
# wget --no-hsts https://copr.fedorainfracloud.org/coprs/kylegospo/bazzite/repo/fedora-"${RELEASE}"/kylegospo-bazzite-fedora-"${RELEASE}".repo -O /etc/yum.repos.d/_copr_kylegospo-bazzite.repo
# wget --no-hsts https://copr.fedorainfracloud.org/coprs/kylegospo/bazzite-multilib/repo/fedora-"${RELEASE}"/kylegospo-bazzite-multilib-fedora-"${RELEASE}".repo?arch=x86_64 -O /etc/yum.repos.d/_copr_kylegospo-bazzite-multilib.repo
...
EXTRA="wezterm rom-properties-gtk3 switcheroo-control system76-scheduler btop wmctrl vulkan-tools steam lutris wine wine-core winetricks protontricks mangohud incus incus-agent jetbrains-mono-fonts-all podman-tui"
